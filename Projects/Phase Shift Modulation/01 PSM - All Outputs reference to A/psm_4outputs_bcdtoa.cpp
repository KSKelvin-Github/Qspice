// Automatically generated C++ file on Tue Aug 19 09:23:13 2025
//
// To build with Digital Mars C++ Compiler:
//
//    dmc -mn -WD psm_4outputs_bcdtoa.cpp kernel32.lib
...
extern "C" __declspec(dllexport) void psm_4outputs_bcdtoa(struct sPSM_4OUTPUTS_BCDTOA **opaque, double t, union uData *data)
{
   ...
   if(!*opaque)
   {
      *opaque = (struct sPSM_4OUTPUTS_BCDTOA *) malloc(sizeof(struct sPSM_4OUTPUTS_BCDTOA));
      bzero(*opaque, sizeof(struct sPSM_4OUTPUTS_BCDTOA));
   }
   struct sPSM_4OUTPUTS_BCDTOA *inst = *opaque;
...
// Implement module evaluation code here:
   ...
   // 2. Outputs A-D can be phase-shifted relative to CLKr
   // 3. minPhaseStp allows negative phase shifts by pre-shifting Output A relative to CLKr
   // 4. All phase shifts are clamped to 0°-359° relative to CLKr
   ...
   if(*ForKeeps & !inst->init)  // Check if initialization is not done yet
   {
      ...
      Display(" - Output B,C,D with phase setpoint reference to A [minPhaseStp,minPhaseStp+360°)\n");
      ...
      Display(" - Valid Phase Setpoint Range for B,C,D: [%.0f°,%.0f°)\n",minPhaseStp,minPhaseStp+360);
   }
   ...
   if(!inst->lastCLKmaster & inst->CLKmaster)  // Detect rising edge
   {
      ...
      // Output B Phase Calculation (PhaseStpB reference to A, inst->PhaseStpB_CLK reference to CLKr)
      inst->PhaseStpB_CLK = PhaseStpB*v2degree-minPhaseStp;  // Phase setpoint B at clock edge, shift [-ve minPhaseStp]
      if (inst->PhaseStpB_CLK > 359) inst->PhaseStpB_CLK = 359;  // max phase = 359o
      if (inst->PhaseStpB_CLK < 0) inst->PhaseStpB_CLK = 0;      // min phase = 0o
      PhBdeg = inst->PhaseStpB_CLK+minPhaseStp;  // Absolute phase for display
      ...
      // Output D Phase Calculation (PhaseStpD reference to A, inst->PhaseStpD_CLK reference to CLKr)
      inst->PhaseStpD_CLK = PhaseStpD*v2degree-minPhaseStp;  // Phase setpoint D at clock edge, shift [-ve minPhaseStp]
      if (inst->PhaseStpD_CLK > 359) inst->PhaseStpD_CLK = 359;  // max phase = 359o
      if (inst->PhaseStpD_CLK < 0) inst->PhaseStpD_CLK = 0;      // min phase = 0o
      PhDdeg = inst->PhaseStpD_CLK+minPhaseStp;
   ...
   // Output B Generation (PhaseStpB reference to CLKr)
   double TphaseB = inst->PhaseStpB_CLK/360*inst->CLKperiod; // Calculate time to PhaseStpB
   if (TfromCLKr >= TphaseB & !inst->triggeredB)   // Check if time since last CLKr equal PhaseB setpoint time & Output B is not yet set
   {
   ...
   if (TfromCLKr >= TphaseD & !inst->triggeredD)   // Check if time since last CLKr equal PhaseD setpoint time & Output D is not yet set
   {
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sPSM_4OUTPUTS_BCDTOA *inst, double t)
{
   return 1e308; // implement a good choice of max timestep size that depends on struct sPSM_4OUTPUTS_BCDTOA
}

extern "C" __declspec(dllexport) void Trunc(struct sPSM_4OUTPUTS_BCDTOA *inst, double t, union uData *data, double *timestep)
{ // limit the timestep to a tolerance if the circuit causes a change in struct sPSM_4OUTPUTS_BCDTOA
   const double ttol = 1e-9; // 1ns default tolerance
   if(*timestep > ttol)
   {
      struct sPSM_4OUTPUTS_BCDTOA tmp = *inst;
      psm_4outputs_bcdtoa(&(&tmp), t, data);
...
extern "C" __declspec(dllexport) void Destroy(struct sPSM_4OUTPUTS_BCDTOA *inst)
{
   free(inst);
}
