// Automatically generated C++ file on Thu Jan  8 08:34:50 2026
//
// To build with Digital Mars C++ Compiler:
//
//    dmc -mn -WD -o franalyzer2ksk.cpp kernel32.lib
...
struct sFRANALYZER2KSK
{
  ...
  double lastI3;     // Previous value of I3
  double lastQ3;     // Previous value of Q3
  double lastI4;     // Previous value of I4
  double lastQ4;     // Previous value of Q4

  // Phase tracking
  double lastPh21;     // Previous phase measurement for phase unwrapping
  double lastPh43;     // Previous phase measurement for phase unwrapping
  ...
  double cumsumI3;   // Accumulated in-phase component for Channel 3
  double cumsumQ3;   // Accumulated quadrature component for Channel 3
  double cumsumI4;   // Accumulated in-phase component for Channel 4
  double cumsumQ4;   // Accumulated quadrature component for Channel 4
...
extern "C" __declspec(dllexport) void franalyzer2ksk(struct sFRANALYZER2KSK **opaque, double t, union uData *data)
{
   double       CH1          = data[ 0].d  ; // input
   double       CH2          = data[ 1].d  ; // input
   double       CH3          = data[ 2].d  ; // input
   double       CH4          = data[ 3].d  ; // input
   double       Tstart       = data[ 4].d  ; // input parameter
   double       fstart       = data[ 5].d  ; // input parameter
   double       fstop        = data[ 6].d  ; // input parameter
   double       fstep        = data[ 7].d  ; // input parameter
   bool         logsweep     = data[ 8].b  ; // input parameter
   double       lvlstart     = data[ 9].d  ; // input parameter
   double       lvlmid       = data[10].d  ; // input parameter
   double       lvlmidfreq   = data[11].d  ; // input parameter
   double       lvlstop      = data[12].d  ; // input parameter
   double       lvlDCoffset  = data[13].d  ; // input parameter
   double       precycles    = data[14].d  ; // input parameter
   double       meascycles   = data[15].d  ; // input parameter
   bool         autocycles   = data[16].b  ; // input parameter
   bool         quickLowFreq = data[17].b  ; // input parameter
   double       LowFreq      = data[18].d  ; // input parameter
   double       LFstep       = data[19].d  ; // input parameter
   bool         plotbode     = data[20].b  ; // input parameter
   const char * fname        = data[21].str; // input parameter
   double      &PertSrc      = data[22].d  ; // output
   double      &SrcFrq       = data[23].d  ; // output
   double      &SrcLvl       = data[24].d  ; // output
   double      &Gain21       = data[25].d  ; // output
   double      &Ph21         = data[26].d  ; // output
   double      &CH1mag       = data[27].d  ; // output
   double      &CH2mag       = data[28].d  ; // output
   double      &Gain43       = data[29].d  ; // output
   double      &Ph43         = data[30].d  ; // output
   double      &CH3mag       = data[31].d  ; // output
   double      &CH4mag       = data[32].d  ; // output
   double      &debug1       = data[33].d  ; // output
   double      &debug2       = data[34].d  ; // output

   if(!*opaque)
   {
      *opaque = (struct sFRANALYZER2KSK *) malloc(sizeof(struct sFRANALYZER2KSK));
      bzero(*opaque, sizeof(struct sFRANALYZER2KSK));
   }
   struct sFRANALYZER2KSK *inst = *opaque;
...
// Implement module evaluation code here:
   ...
   if (t >= Tstart){
      ...
      double I3 = CH3*rsin;   // In-phase component of CH3
      double Q3 = CH3*rcos;   // Quadrature component of CH3
      double I4 = CH4*rsin;   // In-phase component of CH4
      double Q4 = CH4*rcos;   // Quadrature component of CH4
      ...
      if (true){
         ...
         inst->cumsumI3 += (I3+inst->lastI3)/2.0*dT;
         inst->cumsumQ3 += (Q3+inst->lastQ3)/2.0*dT;
         inst->cumsumI4 += (I4+inst->lastI4)/2.0*dT;
         inst->cumsumQ4 += (Q4+inst->lastQ4)/2.0*dT;
         inst->lastI1 = I1;
         inst->lastQ1 = Q1;
         inst->lastI2 = I2;
         inst->lastQ2 = Q2;
   ...
   if (autocycles){
      ...
      if (Gain21 > 3.2) automeascycles = 1;          // Gain : 3.2 (10dB) : fast measurement (less averaging needed)
      else if (Gain21 > 1.0) automeascycles = 2;     // Gain : 1.0 (0dB)
      else if (Gain21 > 0.32) automeascycles = 4;    // Gain : 0.32 (-10dB)
      else if (Gain21 > 0.1) automeascycles = 8;     // Gain : 0.1 (-20dB)
      else if (Gain21 > 0.0) automeascycles = 16;    // Extremely low gain : maximum averaging
   ...
   if (rsin>=0 & inst->lastRsin<0){  // Detect negative-to-positive zero crossing at rsin
      ...
      if (inst->NumRefCycles >= meascycles + precycles){
         ...
         double I3dc = mixerfreq / meascycles * inst->cumsumI3;    // 1/Tperiod/meascycles * int(I3)
         double Q3dc = mixerfreq / meascycles * inst->cumsumQ3;    // 1/Tperiod/meascycles * int(Q3)
         double I4dc = mixerfreq / meascycles * inst->cumsumI4;    // 1/Tperiod/meascycles * int(I4)
         double Q4dc = mixerfreq / meascycles * inst->cumsumQ4;    // 1/Tperiod/meascycles * int(Q4)
         ...
         double A3 = 2*hypot(I3dc,Q3dc);  // CH3 amplitude (factor of 2 for RMS-to-peak)
         double Ph3 = atan2(Q3dc,I3dc);   // CH3 phase (radians)
         double A4 = 2*hypot(I4dc,Q4dc);  // CH4 amplitude
         double Ph4 = atan2(Q4dc,I4dc);   // CH4 phase

         // Calculate output values
         SrcFrq = mixerfreq;  // Pert Src frequency point
         SrcLvl = lvl;        // Pert Src level
         Gain21 = A2/A1;        // Transfer function magnitude : |CH2/CH1|
         Ph21 = Ph2 - Ph1;      // Phase difference (radians) : <CH2-<CH1
         CH1mag = A1;         // CH1 amplitude (factor of 2 for RMS-to-peak)
         CH2mag = A2;         // CH2 amplitude (factor of 2 for RMS-to-peak)
         Gain43 = A4/A3;        // Transfer function magnitude : |CH4/CH3|
         Ph43 = Ph4 - Ph3;      // Phase difference (radians) : <CH4-<CH3
         CH3mag = A3;         // CH3 amplitude (factor of 2 for RMS-to-peak)
         CH4mag = A4;         // CH4 amplitude (factor of 2 for RMS-to-peak)

         // Phase unwrapping (handle -pi to +pi transitions)
         if (inst->lastPh21 < 0 && Ph21 > 0){
            Ph21 = Ph21 - 2*PI;   // Add 2pi to maintain continuity
         }
         // Phase unwrapping (handle -pi to +pi transitions)
         if (inst->lastPh43 < 0 && Ph43 > 0){
            Ph43 = Ph43 - 2*PI;   // Add 2pi to maintain continuity
         }

         // Initialize output window and output files on first measurement
         if (inst->NumMeasCount ==0){
            // Display results : headerlines
            Display("No., Freq(Hz), Gain21(dB), Phase21(°),    |CH1|,    |CH2|,|CH2/CH1|,meascycles, Gain43(dB), Phase43(°)\n");
            ...
            if (*StepNumber==1){    // with .step, only open a write file one time
               ...
               if(*ForKeeps) fprintf(fptr,"\"Frequency\",\"TransferFunction21\",\"TransferFunction43\"\n");
               fclose(fptr);
            }
         }
         // Display results : Frequency, Gain21(dB), Phase21(°)
         Display("%3.0f, %8.0f, %10.2f, %10.2f, %8.3f, %8.3f, %8.3f, %9.0f, %10.2f, %10.2f\n",inst->NumMeasCount,mixerfreq,20*log10(Gain21),Ph21*180/PI,A1,A2,A2/A1,meascycles,20*log10(Gain43),Ph43*180/PI);
         // Save results into .csv - Qspice polar format : Frequency  Real,Imag
         FILE *fptr = fopen(inst->fname,"a");
         if(*ForKeeps) fprintf(fptr,"%.12f\t%.12f,%.12f\t%.12f,%.12f\n",mixerfreq,Gain21*cos(Ph21),Gain21*sin(Ph21),Gain43*cos(Ph43),Gain43*sin(Ph43));
         fclose(fptr);
...
extern "C" __declspec(dllexport) double MaxExtStepSize(struct sFRANALYZER2KSK *inst, double t)
{
   if (!inst->complete)
...
extern "C" __declspec(dllexport) void PostProcess(struct sFRANALYZER2KSK *inst)
{
   if(inst->plotbode){
      // command preparation
...
extern "C" __declspec(dllexport) void Destroy(struct sFRANALYZER2KSK *inst)
{
   free(inst);
}
